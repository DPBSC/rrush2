    ...
    <main className="min-h-screen px-[clamp(1rem,3vw,3rem)] pb-[clamp(4rem,7vw,7rem)] pt-[clamp(5.75rem,8vw,8rem)]">
      <div className="mx-auto grid w-full max-w-[min(94vw,1720px)] gap-[clamp(2rem,4vw,5rem)] lg:grid-cols-[minmax(13rem,0.68fr)_minmax(21rem,1.2fr)_minmax(14rem,0.72fr)] xl:grid-cols-[minmax(15rem,0.72fr)_minmax(28rem,1.1fr)_minmax(16rem,0.72fr)]">
        ...
        <aside className="order-3 self-start lg:sticky lg:top-[clamp(6rem,8vw,8rem)] lg:pt-[clamp(5rem,9vw,9rem)]">
            ...
            <div className="grid grid-cols-4 gap-2">
              {sizes.map((size) => (
                <button
                  ...
                  className={`min-h-[clamp(2.75rem,3.2vw,3.15rem)] border px-3 text-[clamp(0.7rem,0.7vw,0.8rem)] uppercase tracking-[0.18em] transition-colors ${selectedSize === size ? 'border-black bg-black text-[#F7F5F2]' : 'border-black/20 text-black hover:border-black'}`}
          ...
          <button onClick={handlePreOrder} className="mt-2 h-[clamp(3.35rem,4vw,3.8rem)] w-full bg-[#0D0D0D] px-6 text-[clamp(0.7rem,0.72vw,0.82rem)] uppercase tracking-[0.18em] text-[#F7F5F2] transition-colors duration-300 hover:bg-[#1C2533]">
            Pre-Order
          </button>
        </aside>
      </div>
